...
import { ArrowLeft, User, Scissors, CalendarDays, Clock } from "lucide-react"
...
export default function NovoAgendamento() {
  ...
  const [servicoSelecionado, setServicoSelecionado] = useState("Corte")
  ...
  const servicos = [
    ...
    { nome: "Corte + Barba", preco: "R$ 70" },
    { nome: "Sobrancelha", preco: "R$ 20" },
  ]

  const dataFinal = dataCustom || dias[dataSelecionada].dataCompleta
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4">
      ...
      {/* CLIENTE */}
      <div className="mb-6">
        <p className="text-sm text-orange-500 mb-2">1. CLIENTE</p>

        <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 flex items-center gap-3">
          <User className="text-orange-500" />
          <div>
            <p>Cliente</p>
            <p className="text-xs text-gray-400">Nome e número</p>
          </div>
        </div>
      </div>
      ...
      {/* SERVIÇOS */}
      <div className="mb-6">
        <p className="text-sm text-orange-500 mb-2">2. SERVIÇO</p>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {servicos.map((servico) => (
            ...
            >
              ...
              <p className="text-xs text-gray-400">{servico.preco}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* DATA */}
      <div className="mb-6">
        <p className="text-sm text-orange-500 mb-2">3. DATA</p>
          ...
          {dias.map((dia, index) => (
            <div
              ...
              className={`min-w-[70px] text-center p-3 rounded-xl border cursor-pointer
                ${dataCustom === null && dataSelecionada === index
                  ? "bg-orange-500 text-black border-orange-500"
                  : "bg-zinc-900 border-zinc-800"}
              `}
            >
              <p className="text-xs capitalize">{dia.diaSemana}</p>
              <p className="font-bold">{dia.diaNumero}</p>
              <p className="text-[10px]">{dia.mes}</p>
            </div>
          ))}
      ...
      {abrirCalendario && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          ...
          <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800">
            ...
            <button
              onClick={() => setAbrirCalendario(false)}
      ...
      {/* HORÁRIOS */}
      <div className="mb-6">
        <p className="text-sm text-orange-500 mb-2">4. HORÁRIO</p>
      ...
      <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 mb-6">
        <p className="text-sm text-orange-500 mb-2">Resumo</p>
